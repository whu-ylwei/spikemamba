...
class S2DepthTransformerUNetConv(BaseERGB2Depth):
    def __init__(self, config):
        ...
        self.activation = self._build_output_activation()
    ...
    def build_prediction_layer(self):
        self.pred = ConvLayer(self.base_num_channels // 2 if self.skip_type == 'sum' else 2 * self.base_num_channels,
                              self.num_output_channels, 1, activation=None, norm=self.prediction_norm)

    def _build_output_activation(self):
        if self.output_activation in [None, 'none', 'identity']:
            return None
        try:
            return getattr(torch, self.output_activation)
        except AttributeError as exc:
            raise KeyError(
                'Unsupported output_activation: {}. Expected one of '
                '"sigmoid", "softplus", "tanh", "identity", or "none".'.format(self.output_activation)
            ) from exc
    ...
    def forward_decoder(self, super_states):
        ...
        logits = self.pred(x)
        img = logits if self.activation is None else self.activation(logits)
