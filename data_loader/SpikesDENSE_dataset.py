...
class SequenceSynchronizedFramesSpikesDENSEDataset(Dataset):
    """Load sequences of time-synchronized {spike tensors + depth} from a folder."""

    def __init__(self, base_folder, spike_folder, depth_folder='frames', frame_folder='rgb', flow_folder='flow', semantic_folder='semantic/data/',
        ...
        self.dataset = SynchronizedFramesSpikesDENSEDataset(base_folder, spike_folder, depth_folder, frame_folder,
        ...
        self.event_dataset = self.dataset.spike_dataset
...
class SynchronizedFramesSpikesDENSEDataset(Dataset):
    """Loads time-synchronized spike tensors and depth from a folder.

    """

    def __init__(self, base_folder, spike_folder, depth_folder='frames', frame_folder='rgb', flow_folder='flow', semantic_folder='semantic',
                 ...
                 reg_factor=5.7,
                 recurrency=True):
        '''print((base_folder, spike_folder, depth_folder, frame_folder, flow_folder, semantic_folder, \
        ...
        self.spike_dataset = VoxelGridDENSESpikeDataset(base_folder, spike_folder,
        ...
        self.stamps -= self.spike_dataset.initial_stamp
        ...
        '''assert(
            self.stamps[-1] >= self.spike_dataset.get_last_stamp())'''
    ...
    def __getitem__(self, i, seed=None):
        ...
        for k in range(0, self.every_x_rgb_frame):
            ...
            event_timestamp = self.spike_dataset.get_stamp_at(j)
            ...
            # print('dataset index: ', j, ', frame index: ', frame_idx)
            assert(frame_idx >= 0)
            ...
            # Get the spike tensor from the spike dataset loader
            # Note that we pass the transform seed to ensure the same transform is
            if self.baseline != 'rgb':
                events = self.spike_dataset.__getitem__(j, seed)
            ...
            if not bool(self.baseline) or \
                    (self.baseline == 's' and self.loss_composition == "image" and k < self.every_x_rgb_frame - 1):
